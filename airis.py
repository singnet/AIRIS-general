...
class Airis:
    ...
    def predict(self, act, base_state):
        ...
        # self.log.debug('PREDICT All rules / values checked. Updating predict state...')
        for key in predict_s_heap.keys():
            # self.log.debug('PREDICT predict_s_heap key: %s', key)
            for path in predict_s_heap[key].keys():
                # self.log.debug('PREDICT predict_s_heap key path: %s', path)
                # self.log.debug('PREDICT predict_s_heap key path all data: %s', predict_s_heap[key][path])
                try:
                ...
                *head, last = path
                ...
                for i in head:
                    temp = temp[i]
                temp[last] = data[4]
                # self.log.debug('PREDICT post change temp: %s', temp)
                # self.log.debug('PREDICT post change full: %s', predict_state['s_input'][key])

        # Apply unstructured changes
        for key in predict_u_heap.keys():
            # self.log.debug('PREDICT predict_u_heap key: %s', key)
            for path in predict_u_heap[key].keys():
                # self.log.debug('PREDICT predict_u_heap key path: %s', path)
                # self.log.debug('PREDICT predict_u_heap key path all data: %s', predict_u_heap[key][path])
                try:
                ...
                *head, last = path
                ...
                temp[last] = data[4]
                # self.log.debug('PREDICT post change temp: %s', temp)
                # self.log.debug('PREDICT post change full: %s', predict_state['u_input'][key])
        ...
        try:
            ...
            # self.log.debug('predicted state with %s ratio from act / base %s %s', predict_state['match_ratio'], act, base_state)
            for dv in print_state:
                pass
                # self.log.debug(dv)
        except KeyError:
    ...
    def create_rule(self, act, input_type, key, index, pre_val, post_val):
        ...
        # self.knowledge[input_type + '-' + str(key) + '/' + str(pre_val) + '/' + str(new_rule) + '/Indexes'] = [index]
        s_path = []
        for key in self.s_input.keys():
            s_heap = []
        ...
        for path in s_path:
            # self.knowledge[input_type + '-' + str(key) + '/' + str(pre_val) + '/' + str(new_rule) + '/S Conditions/' + str(path[0]) + '/' + str(path[1])] = [path[2]]
    ...
    def list_similarity(self, lst1, lst2):
        if isinstance(lst1, list) and isinstance(lst2, list):
            len_similarity = 1 - abs(len(lst1) - len(lst2)) / max(len(lst1), len(lst2), 1)
            content_similarity = sum(self.list_similarity(a, b) for a, b in zip(lst1, lst2)) / max(len(lst1), len(lst2))
            return (len_similarity + content_similarity) / 2
        elif isinstance(lst1, list) or isinstance(lst2, list):
            return 0  # One is a list, the other is not
        else:
            return SequenceMatcher(None, str(lst1), str(lst2)).ratio()

    def find_best_matches(self, list_1, list_2):
        matched = set()
        matches = {}
        for idx2, lst2 in enumerate(list_2):
            best_match = None
            best_score = -1
            best_idx = None
            for idx1, lst1 in enumerate(list_1):
                if idx1 in matched:
                    continue
                score = self.list_similarity(lst1, lst2)
                if score > best_score:
                    best_match = lst1
                    best_score = score
                    best_idx = idx1
            if best_match and best_score > 0.5:  # Threshold to avoid poor matches
                matches[idx2] = (best_match, lst2, best_idx)
                matched.add(best_idx)
        return matches

    def compare_lists(self, lst1, lst2, path=()):
        changes = []
        if isinstance(lst1, list) and isinstance(lst2, list):
            for i in range(min(len(lst1), len(lst2))):
                changes.extend(self.compare_lists(lst1[i], lst2[i], path + (i,)))
            for i in range(len(lst1), len(lst2)):
                changes.append((None, lst2[i], path + (i,), None, lst2[i]))
            for i in range(len(lst2), len(lst1)):
                changes.append((lst1[i], None, path + (i,), lst1[i], None))
        # elif lst1 != lst2: # record only changes
        else:
            changes.append((lst1, lst2, path, lst1, lst2))
        return changes

    def find_list_change(self, list_1, list_2):
        matches = self.find_best_matches(list_1, list_2)
        changes = []
        checked_indices = set()

        for idx2, (lst1, lst2, idx1) in matches.items():
            checked_indices.add(idx2)
            changes.extend([(lst1, lst2) + change[2:] for change in self.compare_lists(lst1, lst2)])

        for idx2, lst2 in enumerate(list_2):
            if idx2 not in checked_indices:
                changes.append((None, lst2, None, None, None))  # New list detected
